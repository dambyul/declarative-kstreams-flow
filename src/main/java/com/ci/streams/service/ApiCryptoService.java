...
public class ApiCryptoService {
    ...
    private List<String> processWithRetry(List<String> texts, String type, boolean isRetry)
        ...
        if (response.statusCode() == 200) {
            ...
            // Validation: Check 'result' or 'value'
            if (responseRoot.has("result") && !responseRoot.path("result").asBoolean(true)) {
                if (LOG.isErrorEnabled()) {
                    LOG.error("Crypto API returned result=false. Body: {}", response.body());
                }
                throw new RuntimeException("API returned result=false");
            }

            JsonNode outputNode = responseRoot.path("value");
            if (outputNode.isMissingNode() || !outputNode.isArray()) {
                if (LOG.isErrorEnabled()) {
                    LOG.error("Crypto API response missing 'value' array. Body: {}", response.body());
                }
                throw new RuntimeException("API response missing 'value' array");
            }
            ...
            if (LOG.isErrorEnabled()) {
                LOG.error("Crypto API error. Status: {}, Body: {}", response.statusCode(), response.body());
            }
